...
분류모델 성능을 사이킷런을 이용해 정확도(precision)와 재현율(recall)로 확인해본다.
...
확인해보면 0이라고 예측한 데이터의 100%가 실제로 0이었고 1이라고 예측한 데이터의 0%가 실제로 1이었음을 알 수 있다. 또한 실제 0인 데이터 중의 98%가 0으로 판별되었고 실제 1인 데이터 중의 2%만 1로 판별되었음을 알 수 있다. 즉 '지원하지 않는다'에 해당하는 0인 데이터가 압도적으로 많아(skewed) 
