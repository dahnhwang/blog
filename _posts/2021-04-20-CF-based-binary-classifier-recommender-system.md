...
예를 들어 구직자 벡터만을 놓고 설명하면, 데이터셋에 총 3명의 구직자가 있다면 1번 구직자의 벡터는 [1,0,0], 2번 구직자의 벡터는 [0,1,0], 3번 구직자의 벡터는 [0,0,1]이라는 3차원의 원핫 인코딩으로 나타낼 수 있다. 이를 3차원보다 작은 `n_factors` 사이즈로 축소(=데이터 안에서 `n_factors` 개수만큼 사용자를 그룹으로 분류)하게 되는데, (채용공고 벡터와 Dot product를 통해) 가장 실제값과 유사한 예측값을 내놓게되는 구직자 벡터를 찾는 것이 학습의 목표다. 
