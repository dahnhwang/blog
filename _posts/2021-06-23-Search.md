...
'a'라는 이름의 학생은 0번 사물함, 'b' 이름의 학생은 1번 사물함, 마지막 'e' 이름의 학생은 4번 사물함에 배정되었음을 알 수 있다. 이 `paper`라는 딕셔너리로 된 녀석이 해시 테이블로 사용되었음을 알 수 있다. 이 해시 테이블이 있다면 특정 학생의 사물함이 어디인지 굉장히 빠르게 검색해낼 수 있다. 시간복잡도가 O(1)이다.
